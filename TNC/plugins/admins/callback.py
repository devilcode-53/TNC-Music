...
from TNC.utils.database import get_assistant
from TNC.utils.decorators.language import languageCB
from TNC.utils.formatters import seconds_to_min
from TNC.utils.inline import close_markup, stream_markup, stream_markup_timer
from TNC.utils.stream.autoclear import auto_clean
...
async def del_back_playlist(client, CallbackQuery, _):
    ...
    if command == "Pause":
        ...
        if "live_" in queued:
            ...
            try:
                await TNCkip_stream(chat_id, videoid, video=status)
            except:
                return await CallbackQuery.message.reply_text(_["call_6"])
            button = stream_markup(_, chat_id)
            run = await CallbackQuery.message.reply_photo(
